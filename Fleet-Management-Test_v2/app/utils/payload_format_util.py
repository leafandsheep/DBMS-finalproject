...
from app.data.enum.message_enum import MessageType
...
def read_task_payload(topic, payload):
    try:
        ...
        from app.utils.database_util import select_mqtt_gateways
        ...
        mqtt_task = MQTTTask(
            ...
            task_type=TaskType(payload.get('header').get('task_type')),
...
def write_task_payload(
    ...
    try:
        header = {
            'message_id': str(uuid.uuid4()) if message_id is "" else message_id,
